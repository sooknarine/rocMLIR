...
func.func @rock_gemm_schedulev2(%arg0: memref<1x128x128xf16>, %arg1: memref<1x128x115200xf16>, %arg2: memref<1x128x115200xf32>) attributes {block_size = 256 : i32, enable_splitk_for_tuning, grid_size = 3600 : i32, kernel, mhal.arch = "amdgcn-amd-amdhsa:gfx942"} {
  ...
  rock.gridwise_gemm_accel(%arg0, %arg1, %arg2) storeMethod( set) features =  mfma|dot|atomic_add|atomic_add_f16 {arch = "amdgcn-amd-amdhsa:gfx942", blockSize = 256 : i32, gridSize = 3600 : i32, numCU = 228 : i32, params = #rock.xdlops_gemm_derived_params<kpackPerBlock = 8, mPerBlock = 64, nPerBlock = 64, kpack = 8, mPerWave = 32, nPerWave = 32, mnPerXdl = 32, splitKFactor = 1, scheduleVersion = 2, outputSwizzle = 2, forceUnroll = true>} : memref<1x128x128xf16>, memref<1x128x115200xf16>, memref<1x128x115200xf32>
  return
}

// CHECK-LABEL: @rock_gemm_schedulev1
func.func @rock_gemm_schedulev1(%arg0: memref<1x128x128xf16>, %arg1: memref<1x128x115200xf16>, %arg2: memref<1x128x115200xf32>) attributes {block_size = 256 : i32, enable_splitk_for_tuning, grid_size = 3600 : i32, kernel, mhal.arch = "amdgcn-amd-amdhsa:gfx942"} {
    // CHECK: %[[c0:.*]] = arith.constant 0 : index
    // CHECK: memref.store 
    // CHECK-DAG: %[[c1:.*]] = arith.constant 1 : index
    // CHECK-DAG: %[[c2:.+]] = arith.constant 2 : index
  ...
  rock.gridwise_gemm_accel(%arg0, %arg1, %arg2) storeMethod( set) features =  mfma|dot|atomic_add|atomic_add_f16 {arch = "amdgcn-amd-amdhsa:gfx942", blockSize = 256 : i32, gridSize = 3600 : i32, numCU = 228 : i32, params = #rock.xdlops_gemm_derived_params<kpackPerBlock = 8, mPerBlock = 64, nPerBlock = 64, kpack = 8, mPerWave = 32, nPerWave = 32, mnPerXdl = 32, splitKFactor = 1, scheduleVersion = 1, outputSwizzle = 2, forceUnroll = true>} : memref<1x128x128xf16>, memref<1x128x115200xf16>, memref<1x128x115200xf32>
  return
}

// CHECK-LABEL: @rock_conv_gkc01_n01gc_ngk01_0_schedulev2
func.func @rock_conv_gkc01_n01gc_ngk01_0_schedulev2(%arg0: memref<1x32x32xf16>, %arg1: memref<1x32x25600xf16>, %arg2: memref<1x32x25600xf32>) attributes {block_size = 256 : i32, enable_splitk_for_tuning, grid_size = 400 : i32, kernel = 0 : i32, mhal.arch = "amdgcn-amd-amdhsa:gfx942:sramecc+:xnack-"} {
    // CHECK: %[[c0:.*]] = arith.constant 0 : index
    // CHECK: memref.store 
    ...
    // CHECK: name = "MMA"
    // CHECK: pipeline = #rock.pipeline<1>
  rock.gridwise_gemm_accel(%arg0, %arg1, %arg2) storeMethod( set) features =  mfma|dot|atomic_add|atomic_add_f16 {arch = "amdgcn-amd-amdhsa:gfx942:sramecc+:xnack-", blockSize = 256 : i32, gridSize = 400 : i32, numCU = 304 : i32, params = #rock.xdlops_gemm_derived_params<kpackPerBlock = 8, mPerBlock = 32, nPerBlock = 64, kpack = 4, mPerWave = 32, nPerWave = 16, mnPerXdl = 16, splitKFactor = 1, scheduleVersion = 2, outputSwizzle = 2, forceUnroll = true>} : memref<1x32x32xf16>, memref<1x32x25600xf16>, memref<1x32x25600xf32>
  return
}


// CHECK-LABEL: @rock_conv_gkc01_n01gc_ngk01_0_schedulev1
func.func @rock_conv_gkc01_n01gc_ngk01_0_schedulev1(%arg0: memref<1x32x32xf16>, %arg1: memref<1x32x25600xf16>, %arg2: memref<1x32x25600xf32>) attributes {block_size = 256 : i32, enable_splitk_for_tuning, grid_size = 400 : i32, kernel = 0 : i32, mhal.arch = "amdgcn-amd-amdhsa:gfx942:sramecc+:xnack-"} {
    // CHECK: %[[c0:.*]] = arith.constant 0 : index
    // CHECK: memref.store 
    // CHECK-DAG: %[[c1_0:.*]] = arith.constant 1 : index
    // CHECK-DAG: %[[c1_1:.+]] = arith.constant 1 : index
